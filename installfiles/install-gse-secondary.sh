...
install_prerequisites() {
    ...
    /usr/bin/logger "Operating System: $OS Version: $VER: $CODENAME" -t 'gce-2024-04-14';
    echo -e "\e[1;36m...Operating System: $OS Version: $VER: $CODENAME\e[0m";
    # Install prerequisites
    apt-get -qq update > /dev/null 2>&1;
    # Install some basic tools on a Debian net install
    /usr/bin/logger '..Install some basic tools missing if installed from Debian net-install' -t 'gce-2024-04-14';
    ...
    /usr/bin/logger '..Prerequisites for notus-scanner' -t 'gce-2024-04-14';
    ...
    if [ $VER -eq "11" ] 
        ...
        then
            /usr/bin/logger '..install_prerequisites_debian_12_bookworm' -t 'gce-2024-04-14';
            ...
            /usr/bin/logger '....Other prerequisites for Greenbone Community Edition on Debian 12' -t 'gce-2024-04-14';
            apt-get -qq -y install doxygen mosquitto gcc cmake libnet1-dev libglib2.0-dev libgnutls28-dev libpq-dev pkg-config libical-dev xsltproc > /dev/null 2>&1;        
            apt-get -qq -y install software-properties-common libgpgme11-dev uuid-dev libhiredis-dev libgnutls28-dev libgpgme-dev \
                bison libksba-dev libsnmp-dev libgcrypt20-dev gnutls-bin nmap xmltoman gcc-mingw-w64 graphviz rpm nsis \
                sshpass socat gettext python3-polib libldap2-dev libradcli-dev libpq-dev perl-base heimdal-dev libpopt-dev \
    ...
    /usr/bin/logger '....Install preferences on Debian' -t 'gce-2024-04-14';
...
create_scan_user() {
    /usr/bin/logger 'create_scan_user' -t 'gce-2024-04-14';
...
__EOF__
    ...
    /usr/bin/logger 'create_scan_user() finished' -t 'gce-2024-04-14';
...
update_openvas_feed () {
    /usr/bin/logger 'Updating NVT feed database (Redis)' -t 'gce-2024-04-14';
    ...
    /usr/bin/logger 'Updating NVT feed database (Redis) Finished' -t 'gce-2024-04-14';
}
...
create_openvas_version_script() {
    /usr/bin/logger 'create_openvas_version_script()' -t 'gce-2024-04-14';
...
__EOF__
    ...
    /usr/bin/logger 'create_openvas_version_script() finished' -t 'gce-2024-04-14';
