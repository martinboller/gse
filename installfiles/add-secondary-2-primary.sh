...
create_gsecerts() {
    /usr/bin/logger 'create_gsecerts' -t 'gce-2024-12-01';
    ...
    # Create Certs
    /usr/bin/logger 'Creating certificates for secondary' -t 'gce-2024-12-01';
    ...
    if test -f $GVM_CERT_FILENAME; then
        /usr/bin/logger "Successfully created certificates for secondary $SECHOST" -t 'gce-2024-12-01';
        ...
        /usr/bin/logger "Failed creating Certificates for secondary $SECHOST" -t 'gce-2024-12-01';
    ...
    /usr/bin/logger 'create_gsecerts finished' -t 'gce-2024-12-01';
}

add_secondary() {
    /usr/bin/logger 'add_secondary()' -t 'gce-2024-12-01';
    ...
    /usr/bin/logger 'add_secondary() finished' -t 'gce-2024-12-01'
}

show_scanner_status() {
    /usr/bin/logger 'show_scanner_status()' -t 'gce-2024-12-01';
    ...
    if [ $? -eq 0 ]; then
        ...
        echo -e "\e[1;32mdisabling user $GREENBONEUSER on secondary $SECHOST is strongly encouraged\e[0m";
        #sshpass -p $SECPASSWORD ssh -o "StrictHostKeyChecking no" $GREENBONEUSER@$SECHOST "su - gvm -c '/opt/gvm/sbin/openvas --update-vt-info'"
        #sshpass -p $SECPASSWORD ssh -o "StrictHostKeyChecking no" $GREENBONEUSER@$SECHOST "passwd --lock $GREENBONEUSER"
    else
    ...
    /usr/bin/logger 'show_scanner_status() finished' -t 'gce-2024-12-01';
}
...
main() {
    ...
    # Heavy handed clean-up of previously known hosts
    rm ~/.ssh/known_hosts*;
    # Shared components
    echo -e;
