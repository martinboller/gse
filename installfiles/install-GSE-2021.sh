...
install_prerequisites() {
    ...
    /usr/bin/logger '..install some basic tools not available if installed from Debian net install' -t 'gse-21.4.4';
    ...
    /usr/bin/logger '..Prerequisites for Greenbone Security Assistant' -t 'gse-21.4.4';
    ...
    if [ $VER -eq "11" ] 
        ...
        then
            ...
            /usr/bin/logger '..install_prerequisites_debian_Untested' -t 'gse-21.4.4';
    ...
    /usr/bin/logger '....Prerequisites for PDF report generation' -t 'gse-21.4.4';
    echo -e "\e[1;36m ... installing texlive required for PDF report generation\e[0m";
    echo -e "\e[1;36m ... please be patient, this may take quite a while depending on your system\e[0m";
    apt-get -qq -y install texlive-full texlive-fonts-recommended > /dev/null 2>&1;
    # Install other preferences and clean up APT
    /usr/bin/logger '....Install some preferences on Debian and clean up apt' -t 'gse-21.4.4';
...
prepare_nix() {
    /usr/bin/logger 'prepare_nix()' -t 'gse-21.4.4';
...
__EOF__
    ...
    /usr/bin/logger 'prepare_nix() finished' -t 'gse-21.4.4';
}

prepare_source() {    
    /usr/bin/logger 'prepare_source' -t 'gse-21.4.4';
    ...
    wget -O gvm-libs.tar.gz https://github.com/greenbone/gvm-libs/archive/refs/tags/v21.4.4.tar.gz > /dev/null 2>&1;
    /usr/bin/logger '..ospd-openvas' -t 'gse-21.4.4';
    wget -O ospd-openvas.tar.gz https://github.com/greenbone/ospd-openvas/archive/refs/tags/v21.4.4.tar.gz > /dev/null 2>&1;
    /usr/bin/logger '..openvas-scanner' -t 'gse-21.4.4';
    wget -O openvas.tar.gz https://github.com/greenbone/openvas-scanner/archive/refs/tags/v21.4.4.tar.gz > /dev/null 2>&1;
    /usr/bin/logger '..gvm daemon' -t 'gse-21.4.4';
    wget -O gvmd.tar.gz https://github.com/greenbone/gvmd/archive/refs/tags/v21.4.5.tar.gz > /dev/null 2>&1;
    /usr/bin/logger '..gsa daemon' -t 'gse-21.4.4';
    wget -O gsad.tar.gz https://github.com/greenbone/gsad/archive/refs/tags/v21.4.4.tar.gz > /dev/null 2>&1;
    /usr/bin/logger '..gsa webserver' -t 'gse-21.4.4';
    wget -O gsa.tar.gz https://github.com/greenbone/gsa/archive/refs/tags/v21.4.4.tar.gz > /dev/null 2>&1;
    ...
    /usr/bin/logger '..open and extract the tarballs' -t 'gse-21.4.4';
    find *.gz | xargs -n1 tar zxvfp > /dev/null 2>&1;
    sync;

    # Naming of directories w/o version
    /usr/bin/logger '..rename directories' -t 'gse-21.4.4';    
    echo -e "\e[1;36m ... renaming package directories\e[0m";
    mv /opt/gvm/src/greenbone/gvm-libs-21.4.4 /opt/gvm/src/greenbone/gvm-libs > /dev/null 2>&1;
    mv /opt/gvm/src/greenbone/ospd-openvas-21.4.4 /opt/gvm/src/greenbone/ospd-openvas > /dev/null 2>&1;
    mv /opt/gvm/src/greenbone/openvas-scanner-21.4.4 /opt/gvm/src/greenbone/openvas > /dev/null 2>&1;
    mv /opt/gvm/src/greenbone/gvmd-21.4.5 /opt/gvm/src/greenbone/gvmd > /dev/null 2>&1;
    mv /opt/gvm/src/greenbone/gsa-21.4.4 /opt/gvm/src/greenbone/gsa > /dev/null 2>&1;
    mv /opt/gvm/src/greenbone/gsad-21.4.4 /opt/gvm/src/greenbone/gsad > /dev/null 2>&1;
    ...
    /usr/bin/logger 'prepare_source finished' -t 'gse-21.4.4';
}
...
install_gvm_libs() {
    ...
    make > /dev/null 2>&1;
    #/usr/bin/logger '..make gvm libraries Documentation' -t 'gse-21.4.4';
    #make doc-full;
    /usr/bin/logger '..make install Greenbone Vulnerability Manager libraries (gvm-libs)' -t 'gse-21.4.4';
...
prestage_scan_data() {
    ...
    /usr/bin/logger '..opening and extracting TAR Ball' -t 'gse-21.4.4';
...
install_gsad() {
    /usr/bin/logger 'install_gsad' -t 'gse-21.4.4';
    ...
    /usr/bin/logger '..cmake GSA Daemon' -t 'gse-21.4.4';
    echo -e "\e[1;36m ... cmake Greenbone Security Assistant Daemon (GSAD)\e[0m";
    export PKG_CONFIG_PATH=/opt/gvm/lib/pkgconfig:$PKG_CONFIG_PATH;
    cmake -DCMAKE_INSTALL_PREFIX=/opt/gvm . > /dev/null 2>&1;
    /usr/bin/logger '..make GSA Daemon' -t 'gse-21.4.4';
    echo -e "\e[1;36m ... make Greenbone Security Assistant Daemon (GSAD)\e[0m";
    make > /dev/null 2>&1;                # build the libraries
    #/usr/bin/logger '..make documentation for GSA Daemon' -t 'gse-21.4.4';
    #make doc-full;       # build more developer-oriented documentation
    /usr/bin/logger '..make install GSA Daemon' -t 'gse-21.4.4';
    ...
    /usr/bin/logger 'install_gsad finished' -t 'gse-21.4.4';
}

install_gsa_web() {
    /usr/bin/logger 'install_gsa_web()' -t 'gse-21.4.4';
    echo -e "\e[1;32m - install_gsa_web() \e[0m";
    ## Install GSA
    cd /opt/gvm/src/greenbone > /dev/null 2>&1
    chown -R gvm:gvm /opt/gvm > /dev/null 2>&1;
    # GSA prerequisites
    /usr/bin/logger '..installing Yarn' -t 'gse-21.4.4';
    echo -e "\e[1;36m ... installing yarn\e[0m";
    apt-get -qq -y install yarnpkg > /dev/null 2>&1;
    # GSA Install
    cd gsa/ > /dev/null 2>&1;
    /usr/bin/logger '..Build GSA Web Server' -t 'gse-21.4.4';
    ...
    /usr/bin/logger 'install_gsa_web() finished' -t 'gse-21.4.4';
}

browserlist_update(){
    /usr/bin/logger 'browserlist_update()' -t 'gse-21.4.4';
...
__EOF__
    ...
    /usr/bin/logger 'browserlist_update() finished' -t 'gse-21.4.4';
}
...
prepare_postgresql() {
    /usr/bin/logger 'prepare_postgresql' -t 'gse-21.4.4';
    ...
    /usr/bin/logger 'prepare_postgresql finished' -t 'gse-21.4.4';
}
...
configure_gvm() {
    /usr/bin/logger 'configure_gvm' -t 'gse-21.4.4';
...
__EOF__
    ...
    /usr/bin/logger 'configure_gvm() finished' -t 'gse-21.4.4';
}

configure_gsa() {
    /usr/bin/logger 'configure_gsa' -t 'gse-21.4.4';
...
/usr/bin/logger 'configure_gsa finished' -t 'gse-21.4.4';
}

configure_feed_owner() {
    /usr/bin/logger 'configure_feed_owner' -t 'gse-21.4.4';
    ...
    then
        ...
        /usr/bin/logger 'configure_feed_owner User creation success' -t 'gse-21.4.4';
    else
        echo "User admin for GVM $HOSTNAME could NOT be created - FAIL!" >> /var/lib/gvm/adminuser;
        /usr/bin/logger 'configure_feed_owner User creation FAILED!' -t 'gse-21.4.4';
    fi
    echo -e "\e[1;32m - configure_feed_owner() finished\e[0m";
    /usr/bin/logger 'configure_feed_owner finished' -t 'gse-21.4.4';
}
...
start_services() {
    ...
    then
        ...
        /usr/bin/logger 'gvmd.service started successfully' -t 'gse-21.4.4';
    else
        echo -e "\e[1;31mgvmd.service FAILED!\e[0m";
        /usr/bin/logger 'gvmd.service FAILED' -t 'gse-21.4.4';
    fi
    # gsad.service
    if systemctl is-active --quiet gsad.service;
    then
        echo -e "\e[1;32mgsad.service started successfully";
        /usr/bin/logger 'gsad.service started successfully' -t 'gse-21.4.4';
    else
        echo -e "\e[1;31mgsad.service FAILED!\e[0m";
        /usr/bin/logger "gsad.service FAILED!" -t 'gse-21.4.4';
    fi
    ...
    then
        ...
        /usr/bin/logger 'ospd-openvas.service started successfully' -t 'gse-21.4.4';
    else
        echo -e "\e[1;31mospd-openvas.service FAILED!";
        /usr/bin/logger 'ospd-openvas.service FAILED!\e[0m' -t 'gse-21.4.4';
    fi
...
get_scanner_status() {
    /usr/bin/logger 'get_scanner_status()' -t 'gse-21.4.4';
    ...
    /usr/bin/logger ''Default OpenVAS $(su gvm -c "/opt/gvm/sbin/gvmd --verify-scanner 08b69003-5fc2-4037-a479-93b440211c73")'' -t 'gse-21.4.4';    
    /usr/bin/logger ''Default CVE $(su gvm -c "/opt/gvm/sbin/gvmd --verify-scanner 6acd0832-df90-11e4-b9d5-28d24461215b")'' -t 'gse-21.4.4';
...
configure_cmake() {
    /usr/bin/logger 'configure_cmake' -t 'gse-21.4.4';
   ...
   /usr/bin/logger 'configure_cmake finished' -t 'gse-21.4.4';
}
...
main() {
    ...
    echo -e "\e[1;32m-----------------------------------------------------------------------------------------------------\e[0m"
    echo -e "\e[1;36m ... Starting installation of primary Greenbone Source Edition Server 21.4.4\e[0m"
    ...
    configure_feed_owner;
    get_scanner_status;
    /usr/bin/logger 'Installation complete - Give it a few minutes to complete ingestion of feed data into Postgres/Redis, then reboot' -t 'gse-21.4.4';
    echo -e;
