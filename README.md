...
- This server needs at least 4Gb of RAM, preferably more
...
The first OpenVas scanner is always this UUID: 08b69003-5fc2-4037-a479-93b440211c73
The script verifies bot the OpenVAS and the GVMD Scanner bu running
```
...
They should return this (Versions as of Oct. 2021)

<i>Scanner version: OpenVAS 21.4.3.

Scanner version: GVM/21.4.4. </i>
...
So do <b>not</b> delete this account, unless you reconfigure it to be another. <b>Do</b> remember to change its initial password as discussed here.
<b>Without a feed owner there will be no feeds!!</b> (ask me how I know)


If you want to change feedowner, the following commands can be used to create another account. You can also just change it in install-GSE-2021.sh before running it the first time.
...
Just after installation, going from nothing in the feeds, you'll notice that postgres is being hammered by gvmd and that redis are too, but by ospd-openvas, as the NVTs are stored in Redis (on the secondary it is only Redis). When feeds are updated this isn't as obvious, as the delta is way less.
